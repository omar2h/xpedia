...
std::vector<FlightOffer> DuffelMapper::toFlightOffers(const DuffelResponseDTO &response)
{
    ...
    for (std::size_t oi = 0; oi < response.offers.size(); ++oi)
    {
        ...
        for (std::size_t si = 0; si < src.slices.size(); ++si)
        {
            ...
            if (sliceDto.segments.empty())
            {
                ...
                seg.from = sliceDto.originIata;
                seg.to = sliceDto.destinationIata;
            ...
            for (const auto &segDto : sliceDto.segments)
            {
                ...
                seg.from = segDto.originIata;
                seg.to = segDto.destinationIata;
                ...
                seg.checkedBags = extractCheckedBags(response, oi, si);
                offer.segments.push_back(std::move(seg));
            }
        }
