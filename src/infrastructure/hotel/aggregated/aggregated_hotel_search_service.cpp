...
AggregatedHotelSearchService::AggregatedHotelSearchService(
...
Result<std::vector<HotelOffer>> AggregatedHotelSearchService::searchHotels(const HotelSearchRequest &request)
{
    ...
    if (!anySucceeded)
    {
        return Result<std::vector<HotelOffer>>::failure(
            errors.empty() ? "All hotel search providers failed" : errors);
    }
