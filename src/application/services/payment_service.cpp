...
PaymentProcessor::PaymentProcessor(ICustomerRepository& customerRepo,
    ...
    : m_customerRepo(customerRepo), m_getPaymentService(std::move(getPaymentService)),
...
bool PaymentProcessor::withdrawMoney(const Customer& customer, double amount, int service)
{
